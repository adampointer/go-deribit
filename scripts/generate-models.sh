...
for p in private public; do
    for f in examples/${p}/*.request.json; do
        ...
        cat ${f} | jq .params | gojson -name="${struct}" -tags=json,mapstructure -pkg=${p} -o=models/${p}/${name%.*}_request.go
        if [[ $? -ne 0 ]]; then
    ...
    for f in examples/${p}/*.response.json; do
        ...
        cat ${f} | jq .result | gojson -name="${struct}" -tags=json,mapstructure -pkg=${p} -o=models/${p}/${name%.*}_response.go
        if [[ $? -ne 0 ]]; then
...
for f in examples/notifications/*.json; do
    ...
    cat ${f} | jq .params.data[0] | gojson -name="${struct}" -tags=json,mapstructure -pkg=notifications -o=models/notifications/${name}.go
done